<template>
  <div>
    ...
    <div class="main-content">
      ...
      <p>To get a better view of this distortion, select “Highlight Face” in the current scene. You have to really force yourself to think in 2D for this one, but observe how the face's shape changes in the projection image during a rotation. While a cube's actual face is always a square with 90° angles, the distorted angles can make the projection be a square, trapezoid, or any 4 sided shape in between depending on the cube’s orientation.</p>
      ...
      <p>You can boost its effectiveness even further by highlighting an object's different points, edges, or faces to reveal different insights. Look at how when we don't highlight anything, the projection of a solid sphere doesn't appear to change during rotation:</p>
      <SceneButtonSingle :sceneID="scenes.threeandcanvas.projSphere" :text="'LOAD SCENE'" :imgName="'proj_3dsphere.png'"></SceneButtonSingle>
      <p>If instead we highlight points along the surface of the sphere, we can now observe how the surface moves during rotation:</p>
